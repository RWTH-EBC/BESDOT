...
class Battery(Storage):
    ...
    def _constraint_conver(self, model):
        """
        Rewrite the energy conversion equation for battery. Compared with
        Storage the energy loss is considered. In first version the loss in
        each hour is seen as a fixed portion of stored energy.
        """
        input_energy = model.find_component('input_' + self.inputs[0] +
                                            '_' + self.name)
        output_energy = model.find_component('output_' + self.outputs[0] +
        ...
        for t in range(len(model.time_step)-1):
            model.cons.add(stored_energy[t+1] * (1 - self.loss) + input_energy[
                t+1] - output_energy[t+1] == stored_energy[t+2])
