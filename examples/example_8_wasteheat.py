...
prj = Project(name='test_8', typ='building')
...
bld_1.demand_profile['heat_demand'] = [100] * 8760
source_profile = [30] * 8760

# Pre define the building energy system with the topology for different
# components and add components to the building.
topo_file = os.path.join(base_path, 'data', 'topology', 'heat_source_low.csv')
bld_1.add_topology(topo_file)
bld_1.add_components(prj.environment)

for item in bld_1.components:
    if bld_1.components[item].component_type == 'HeatSource':
        bld_1.components[item].update_profile(source_profile=source_profile)
